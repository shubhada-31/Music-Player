...
public class ArgProgressView extends RelativeLayout {
    ...
    private void init(Context context) {
        ...
        this.tvMessage = findViewById(R.id.tvProgressMessage);
        this.ivHour = findViewById(R.id.imgViewProgressHour);
        this.ivMinute = findViewById(R.id.imgViewProgressMinute);
    ...
    public void start() {
        if (isAnimationsInitialized()) {
            ...
            if (maxTime != -1) {
                new Handler().postDelayed(this::stop, maxTime);
            }
        } else {
            initAnimations();
            start();
